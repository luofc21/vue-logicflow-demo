<template>
  <div>
    ...
    <el-button-group>
      ...
      <el-button type="primary" size="small" @click="downloadXml">下载CMMN</el-button>
    ...
    >
      <el-button type="primary" size="small">导入CMMN</el-button>
    </el-upload>
  </div>
</template>
<script setup>
...
const downloadXml = () => {
  ...
  // 检查图形数据是否存在
  if (!graphData || !props.lf) {
    console.error('图表数据不完整', graphData)
    ElMessage.error('当前图表数据无效')
    return
  }
  
  try {
    // 下载XML文件
    download('logic-flow.cmmn.xml', graphData)
  } catch (error) {
    console.error('转换或下载过程中出错:', error)
    ElMessage.error('导出CMMN文件失败: ' + error.message)
  }
}
