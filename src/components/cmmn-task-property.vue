<template>
  <el-drawer :modelValue="true" @close="hide" title="节点配置" size="50%" destroy-on-close>
    <el-form ref="modelRef" :rules="rules" :key="nodeData.id" :model="model" v-bind="$attrs" label-position="left"
      ...
      <!-- <el-form-item label="类型" prop="auto">
        <el-select v-model="model.auto" @change="handleJobChange" filterable remote reserve-keyword
          :remote-method="queryJobs" :loading="jobLoading">
          <el-option v-for="k in jobOptions" :key="k.name" :label="k.name" :value="k.value" />
        </el-select>
      </el-form-item> -->

      <!-- Action属性编辑区域 -->
      <el-divider content-position="left">任务属性</el-divider>
      <el-form-item label="任务类型" prop="action.taskType">
        <el-select v-model="model.action.taskType" @change="handleJobChange" filterable remote reserve-keyword
          :remote-method="queryJobs" :loading="jobLoading">
          <el-option v-for="k in jobOptions" :key="k.name" :label="k.name" :value="k.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="任务ID" prop="action.taskId">
        <el-input v-model="model.action.taskId"></el-input>
      </el-form-item>
      <el-form-item label="任务名称" prop="action.taskName">
        <el-input v-model="model.action.taskName"></el-input>
      </el-form-item>
      <el-form-item label="请求地址" prop="action.requestUrl">
        <el-input v-model="model.action.requestUrl"></el-input>
      </el-form-item>
      <el-form-item label="请求方法" prop="action.requestMethod">
        <el-select v-model="model.action.requestMethod">
      ...
      <el-form-item label="请求头" prop="action.requestHeaders">
        <el-input v-model="model.action.requestHeaders" type="textarea" :rows="2"></el-input>
      </el-form-item>
      <el-form-item label="主题" prop="action.topic">
        <el-input v-model="model.action.topic"></el-input>
      </el-form-item>
      <el-form-item label="消息" prop="action.massage">
        <el-input v-model="model.action.massage" type="textarea" :rows="2"></el-input>
      </el-form-item>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 确保action对象存在
    oldProperties.action = oldProperties.action || {
      taskType: 'auto',
      requestUrl: 'http://dev-k8s.pcitech.com:30780',
      requestHeaders: '{}',
      requestMethod: 'post',
      topic: 'task',
      massage: 'a normal task',
      taskName: 'taskName',
    ...
    const jobOptions = ref([{ name: '手动', value: 'manual' }, { name: '自动', value: 'auto' }]);
