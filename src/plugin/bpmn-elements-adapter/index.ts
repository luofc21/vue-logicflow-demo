...
function getLfNodes(value: any, shapes: any, key: any) {
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((val) => {
      ...
      if (Array.isArray(shapes)) {
        shapeValue = shapes.find(
          (shape) => shape && shape['-bpmnElement'] === val['-id'],
        );
      } else {
        shapeValue = shapes;
      }
      try {
        ...
        if (node) {
          ...
          console.warn(`[getLfNodes] getNodeConfig返回空值 - id: ${val['-id']}, key: ${key}`);
        }
      } catch (error) {
        console.error(`[getLfNodes] 处理节点时发生错误 - id: ${val['-id']}, key: ${key}`, error);
      }
    });
  } else {
    let shapeValue;
    if (Array.isArray(shapes)) {
      shapeValue = shapes.find(
        (shape) => shape && shape['-bpmnElement'] === value['-id'],
      );
    } else {
      shapeValue = shapes;
    }
    ...
    try {
      ...
      if (node) {
        ...
        console.warn(`[getLfNodes] getNodeConfig返回空值 - id: ${value['-id']}, key: ${key}`);
      }
    } catch (error) {
      console.error(`[getLfNodes] 处理节点时发生错误 - id: ${value['-id']}, key: ${key}`, error);
    }
  }
  return nodes;
}
...
function getNodeConfig(shapeValue: any, type: any, processValue: any) {
  try {
    // 增加边界检查
    if (!shapeValue || !shapeValue['dc:Bounds']) {
      console.warn('Missing shape value or dc:Bounds for node:', processValue);
      return null;
    }

    // 提供默认值
    const bounds = shapeValue['dc:Bounds'] || {
      '-x': 0,
      '-y': 0,
      '-width': 100,
      '-height': 80
    ...
    const nodeConfig: NodeConfig = {
      id: shapeValue['-bpmnElement'] || processValue['-id'],
      type,
      x,
      y,
      properties,
    ...
    console.error('Error in getNodeConfig:', error);
    return null;
  }
}
